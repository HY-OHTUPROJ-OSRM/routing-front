...
import { addTempRoad } from '../../reducers/TempRoadsSlice';
import { getDisconnections } from '../../services/DisconnectionsService';

const DisconnectionModal = ({ onClose }) => {
    const dispatch = useDispatch();
    const disconnectedRoadHandlers = useSelector((state) => state.disconnectedRoad);
    ...
    const handleCreateTempRoad = async (disconnection) => {
        ...
        const getReadableName = (nameA, nameB) => {
            const clean = (name) => (name && name !== '(unnamed)' ? name : 'unnamed');
    ...
    const applyFilters = (disconnections, searchTerm, filterType) => {
        return disconnections.filter((item) => {
            const matchesSearch =
                item.startNode.way_name.toLowerCase().includes(searchTerm) ||
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div
                ...
                style={{
                    width: disconnections.length > 0 ? '80vw' : 'auto',
                    maxHeight: '80vh',
                    overflowY: 'auto',
                }}
            >
                ...
                <div className="button-container">
                    <button onClick={handleGetDisconnections} className="disconnection-button">
                        Get disconnections
                    </button>

                    <button
                        onClick={disconnectedRoadHandlers.clear}
                    ...
                    <button
                        onClick={disconnectedRoadHandlers.list}
                        className="disconnection-button"
                    >
                        Set nodelist
                    </button>
                        ...
                        <div className="search-container">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setFilteredDisconnections(
                                        applyFilters(disconnections, newSearchTerm, filterType)
                                    );
                                }}
                                style={{
                                    width: '100%',
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    setFilteredDisconnections(
                                        applyFilters(disconnections, searchTerm, newFilterType)
                                    );
                                }}
                                style={{
                                    marginLeft: '1rem',
                                ...
                                {filteredDisconnections.map((item, index) => (
                                    <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
                                        <td style={{ padding: '8px', textAlign: 'center' }}>
                                            <div>
                                                A: {item.startNode.id} {item.startNode.way_name}
                                            </div>
                                            <div>
                                                B: {item.endNode.id} {item.endNode.way_name}
                                            </div>
                                        </td>
                                        <td style={{ padding: '8px', textAlign: 'center' }}>
